...
export default function InscriptionPage() {
  ...
  useEffect(() => {
    void listCategories()
      .then((c) => { setCategories(c); if (c[0]) setCategoryId(c[0].id); })
      .catch(() => {});
  }, []);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await register({ email, password, name, city, phone, address: address || undefined, categoryId });
      ...
      <div className="rounded-xl border border-zinc-100 bg-white p-8 shadow-sm">
        ...
        <form onSubmit={onSubmit} className="mt-6 grid gap-4 sm:grid-cols-2">
          <FormField label="Nom de l'enseigne" className="sm:col-span-2">
            <Input required placeholder="Club Padel Paris" value={name} onChange={(e) => setName(e.target.value)} />
          </FormField>
          <FormField label="E-mail" className="sm:col-span-2">
            <Input type="email" required placeholder="contact@club.fr" value={email} onChange={(e) => setEmail(e.target.value)} />
          </FormField>
          ...
          <FormField label="Ville">
            <Input required placeholder="Paris" value={city} onChange={(e) => setCity(e.target.value)} />
          </FormField>
          <FormField label="Téléphone">
            <Input required placeholder="+33 6 00 00 00 00" value={phone} onChange={(e) => setPhone(e.target.value)} />
          </FormField>
          <FormField label="Adresse (optionnel)" className="sm:col-span-2">
            <Input placeholder="12 rue du Padel, 75001 Paris" value={address} onChange={(e) => setAddress(e.target.value)} />
          </FormField>
          ...
          <Button type="submit" loading={loading} className="sm:col-span-2 w-full py-2.5">
            Créer mon compte
          </Button>
        </form>
      </div>
