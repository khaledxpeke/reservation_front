...
export function ResourceAvailabilityForm({
  resourceId,
  resourceName,
  showBackLink = false,
  ...
  const onSave = async (e: React.FormEvent) => {
    ...
    if (avail.length === 0) {
      saveMut.setError("Activez au moins un jour.");
      return;
    }
    setMessage(null);
  ...
  return (
    <div>
      ...
      <div className={showBackLink ? "mt-4" : ""}>
        <PageHeader
          title={resourceName ? `Disponibilités · ${resourceName}` : "Disponibilités"}
          description="Activez les jours et configurez les plages horaires pour les réservations en ligne."
        />
      </div>
          ...
          {DAYS.map(({ key, label, abbr, weekend }) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-sm font-semibold transition ${row.enabled ? "text-zinc-900" : "text-zinc-400"}`}
                >
                  {label}
                </span>
